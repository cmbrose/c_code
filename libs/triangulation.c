...
arraylist_p do_triangulation(arraylist_p points)
{
    ...
    */
    ...
    triangle_t *super_tri = get_super_triangle(points);

    arraylist_p tris = arraylist_make(NULL);
    arraylist_append(tris, super_tri);
    ...
    for (i = 0; i < points->size; ++i)
    {
        ...
        for (j = 0; j < tris->size; ++j)
        {
            ...
            if (point_in_circumcircle(*p, *t))
            {
                ...
                // Need to save this for corner removal
                if (t != super_tri)
                    free(t);
            }
        }
    ...
    {
        triangle_t* tri = arraylist_get(tris, i);

        for ( j = 0; j < 3; ++j )
        {
            point_t p = (*super_tri)[j];

            if ( index_in_triangle(p, *tri) != -1 )
            {
                arraylist_remove(tris, i--);
                free(tri);
                break;
            }
        }
