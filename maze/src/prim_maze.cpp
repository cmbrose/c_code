    ...
    graph &g)
{
    ...
    for (short y = 0; y < height; ++y)
    {
        for (short x = 0; x < width; ++x)
        {
            pixel_t color = ppm_get_pixel(image, x, y);

            // TODO, make this an option. For now, just treat (mostly) black pixels as not in the shape
...
unsigned short **make_maze_in_image(
    ...
    int *out_height)
{
    ppm_t *image = ppm_make_from_file(image_path);
    *out_width = image->w;
    *out_height = image->h;

    graph g;
    make_graph_from_image(*image, weight_variance, g);

    ppm_free(image);

    return make_maze_from_graph(*out_height, *out_width, weight_variance, max_dist, wrap_maze, g);
}
